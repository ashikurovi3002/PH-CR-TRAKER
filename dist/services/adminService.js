...
class AdminService {
    ...
    static async updateProfile(adminId, data) {
        ...
        if (data.profileImage)
            dataToUpdate.profileImage = data.profileImage;
        if (data.institutionType)
            dataToUpdate.institutionType = data.institutionType;
        if (data.password) {
    ...
    static async getUsers(adminId) {
        ...
        const whereClause = { role: 'ambassador' };
    ...
    static async getDashboardStats(adminId) {
        ...
        const [totalUsers, totalActivities, pendingRedeems, totalFeedbacks, pendingFeedbacks, totalBanners] = await Promise.all([
    ...
    // Points
    static async bulkAddPoints(data) {
        return await prisma.$transaction(async (prisma) => {
            ...
            const updatedUsers = await Promise.all(data.userIds.map(userId => prisma.user.update({
                where: { id: userId },
    ...
    static async getPointsHistory(adminId) {
        const admin = await prisma.user.findUnique({ where: { id: adminId } });
        const whereClause = {};
        if (admin && admin.institutionType) {
    ...
    // Redeems
    static async getRedeems(adminId) {
        const admin = await prisma.user.findUnique({ where: { id: adminId } });
        const whereClause = {};
        if (admin && admin.institutionType) {
            whereClause.user = { institutionType: admin.institutionType };
    ...
    static async updateRedeemStatus(id, status) {
        if (status === 'REJECTED') {
    ...
    // Feedbacks
    static async getFeedbacks(adminId) {
        const admin = await prisma.user.findUnique({ where: { id: adminId } });
        const whereClause = {};
        if (admin && admin.institutionType) {
            whereClause.user = { institutionType: admin.institutionType };
